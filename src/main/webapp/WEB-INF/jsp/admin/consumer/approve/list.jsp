...
<m:Content contentPlaceHolderId="content">
	...
	<div style="padding: 15px;" class="layui-anim layui-anim-upbit">
		...
		<div class="layui-tab layui-tab-brief" id="detailPanel" style="display: none;">
		  <ul class="layui-tab-title" id="tab_btn">
		    <li data-taburl="/${applicationScope.adminprefix }/consumer/selUserMsg" class="layui-this">专家信息</li>
	...
	<script type="text/html" id="caozuo">
		...
		<a class="layui-btn layui-btn-xs layui-btn-green" lay-event="selects">查看</a>
	...
	<script type="text/javascript">
		...
		layui.use(['laypage', 'layer', 'table', 'form', 'laydate'], function(){
			...
			var tableIns = table.render({
				id: 'consumer',
				elem: '#consumer',
				url: '/${applicationScope.adminprefix }/specialColumn/listData', //数据接口
				where: {},
				page: true, //开启分页
				...
				cols: [
					...
					$('tbody').on('click','td',function() {
						...
						//获取当前行的id值
						var id = $(this).parents('tr').find('td:eq(9)').find('input[name="userId"]').val();
						if(id != null){
			...
			$(".download").click(function(){
				...
				window.location.href="/${applicationScope.adminprefix }/specialColumn/download?name="+name+"&registrationDate="+registrationDate+"&ids="+ids;
			})
			//搜索，重置表格
			...
			function deletes(userId){
				layer.confirm('确定删除吗？', {icon: 7}, function(){
					$.ajax({
						type : "POST",
						url : "/${applicationScope.adminprefix }/specialColumn/deletes",
						data : {"userId" : userId},
						success : function(data) {
							tableIns.reload({
								where: { //设定异步数据接口的额外参数，任意设
			...
			//查看
			function selMsg(userId){
				layer.open({
					type: 2,
					title: ['', 'font-size:18px;'],
					shadeClose: true,
					area: ['100%', "100%"],
					content: '/${applicationScope.adminprefix }/specialColumn/selConsumer?userId=' + userId ,
					success: function(layero, index) {
						...
						table.reload('consumer', {
							where: {
								approve:$("#tab_list > li.layui-this").data("state"),
								num:Math.random()
							},
							page: {
								curr: 1
			...
			$("#tab_list > li[data-state]").on("click",function(){
				...
				//获取状态值
				var approve = $("#tab_list > li.layui-this").data("state");
				//重新加载表格
				tableIns.reload({
					where: {
						approve:approve,
