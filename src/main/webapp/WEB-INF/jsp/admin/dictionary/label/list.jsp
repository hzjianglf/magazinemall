...
<m:Content contentPlaceHolderId="content">
	...
	<div style="padding: 15px;" class="layui-anim layui-anim-upbit">
		...
		<div class="layui-form">
			<table class="layui-table" lay-skin="line" id="labels" lay-filter="tableContent"></table>
		</div>
		<div id="demo7"></div>
	</div>
	...
	<script type="text/javascript">
		...
		layui.use(['laypage', 'layer', 'table', 'form'], function(){
			...
			var tableIns = table.render({
				id: 'labels',
				elem: '#labels',
				url: '/${applicationScope.adminprefix }/labels/listData', //数据接口
				where: {},
				page: true, //开启分页
			...
			table.on('tool(tableContent)', function(obj){
				...
				if(layEvent === 'edits'){
					openwindow("labels/toaddOrUp?id="+id,"新增/编辑标签",500,400,false,callback);
			...
			//创建标签
			$(".add").click(function(){
				openwindow("labels/toaddOrUp","新增/编辑标签",500,400,false,callback);
			})
			function callback(){
				tableIns.reload({
					where: { //设定异步数据接口的额外参数，任意设
			...
			function batchUpStatus(ids,status,layer, tableIns){
				$.ajax({
					type : "POST",
					url : "/${applicationScope.adminprefix }/labels/batchUpStatus",
					async : false,
			...
			function updateStatus(id,status){
				...
				layer.confirm(text, {icon: 7}, function(){
					$.ajax({
						type : "POST",
						url : "/${applicationScope.adminprefix }/labels/updateStatus",
			...
			function deletes(id){
				layer.confirm('确定删除吗？', {icon: 7}, function(){
					$.ajax({
						type : "POST",
						url : "/${applicationScope.adminprefix }/labels/deletes",
						data : {"id" : id},
						success : function(data) {
							tableIns.reload({
								where: { //设定异步数据接口的额外参数，任意设
			...
			function deleteid(ids,layer, tableIns){
				$.ajax({
					type : "POST",
					url : "/${applicationScope.adminprefix }/labels/deleteids",
					async : false,
					data : {
						"ids" : ids
					},
			...
			$(".download").click(function(){
				...
				window.location.href="/${applicationScope.adminprefix }/labels/download?ids="+ids+"&name="+name+"&status="+status;
			})
		})
