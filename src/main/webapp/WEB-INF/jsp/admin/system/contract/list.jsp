...
<m:Content contentPlaceHolderId="content">
	...
	<div style="padding: 15px;" class="layui-anim layui-anim-upbit">
		...
		<div class="layui-form">
			<table class="layui-table" lay-skin="line" id="contractList" lay-filter="tableContent"></table>
		</div>
		<div id="demo7"></div>
	</div>
	...
	<script type="text/javascript">
		...
		layui.use(['laypage', 'layer', 'table', 'form'], function(){
			...
			var tableIns = table.render({
				id: 'content',
				elem: '#contractList',
				url: '/${applicationScope.adminprefix }/system/contract/getContractList', //数据接口
				page: true, //开启分页
			...
			table.on('tool(tableContent)', function(obj){
				...
				if(layEvent === 'del'){ //删除
			    	layer.confirm('确定删除当前数据吗？', {icon: 7}, function(index){
			      		...
			      		layer.close(index);
			      		//向服务端发送删除指令
			      		delContent(data.id, layer, tableIns);
			    	});
			  		...
			  		layer.open({
			  			...
			  			title: '合同修改',
			  			area: ['100%', '100%'],
			  			content: ['/${applicationScope.adminprefix }/system/contract/getContractInfo?id=' +data.id, 'no'],
			  			end: function(){
			  				tableIns.reload({
								where: { //设定异步数据接口的额外参数，任意设
									num:Math.random()
			...
			$('#add_btn').click(function(){
				layer.open({
		  			...
		  			title: '合同添加',
		  			area: ['100%', '100%'],
		  			content: ['/${applicationScope.adminprefix }/system/contract/add', 'no'],
		  			end: function(){
		  				tableIns.reload({
							where: { //设定异步数据接口的额外参数，任意设
								num:Math.random()
		...
		// 删除
		function delContent(id, layer, tableIns){
			$.ajax({
				type : "POST",
				url : "delContract",
				async : false,
