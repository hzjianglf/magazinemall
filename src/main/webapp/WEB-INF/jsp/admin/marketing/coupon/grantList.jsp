	...
	<script type="text/javascript">
		...
		layui.use(['laypage', 'layer', 'table', 'form', 'laydate'], function(){
			...
			var tableIns = table.render({
				id: 'buyerList',
				elem: '#buyerList',
				url: '/${applicationScope.adminprefix }/coupon/userinfoList?couponId='+$("#couponId").val(), //数据接口
				cellMinWidth: 100,
				page: true, //开启分页
			...
			$("#batchGrant").click(function(){
				...
				if(checkLength > couponCount){
					layer.msg('发放数量超过优惠券剩余数量！', {icon: 7});
					return false;
				}
				var userIds = '';
			...
			function grantCoupon(userId,grantCount){//用户id，发放数量
				layer.confirm('确定发放优惠券吗？', {icon: 7}, function(){
					$.ajax({
						type : "POST",
						url : "/${applicationScope.adminprefix }/coupon/grantCoupon",
						data : {"userIds":userId,"grantCount":grantCount,"couponId":$("#couponId").val()},
