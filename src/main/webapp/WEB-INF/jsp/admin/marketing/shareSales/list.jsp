	...
	<script type="text/javascript">
		...
		layui.use(['laypage', 'layer', 'table', 'form' ,'laydate'], function(){
			...
			var tableIns = table.render({
				id: 'sharesales',
				elem: '#sharesales',
				url: '/${applicationScope.adminprefix }/sharesales/listData', //数据接口
				where: {},
				page: true, //开启分页
			...
			table.on('tool(tableContent)', function(obj){
				...
				if(layEvent === 'details'){
			  		//详细
					openwindow("sharesales/toaddOrUp?id="+id+"&detail=1","详细",600,700,true,callback);
			  	}else if(layEvent === 'edits'){
			  		//编辑
			  		openwindow("sharesales/toaddOrUp?id="+id,"新增/编辑活动",600,700,true,callback);
			...
			//添加营销活动
			$("#add").click(function(){
				//window.location.href="/${applicationScope.adminprefix }/sharesales/toaddOrUp";
				openwindow("sharesales/toaddOrUp","新增/编辑活动",600,700,true,callback);
			})
			...
			function deletes(id){
				layer.confirm('确定删除吗？', {icon: 7}, function(){
					$.ajax({
						type : "POST",
						url : "/${applicationScope.adminprefix }/sharesales/deletes",
						data : {"id" : id},
