	...
	<form action="/${applicationScope.adminprefix }/content/saveContent?num=<%=Math.random() %>" id="contentInfo" class="layui-form" method="post">
		<div class="layui-tab">
			<div class="layui-tab-content" style="background: #FFFFFF;">
				<input type="hidden" name="catId" value="${catId!=null?catId:contentMap.catId }"/>
		 		<input type="hidden" name="contentId" value="${contentMap.contentId }"/>
		 		 <input type="hidden" name="oldPicUrl" id="oldPicUrl" value="${contentMap.picUrl }"/>
				...
				<div class="layui-form">
					<div class="layui-form-item">
					    ...
					    <select name="parentId" id="parentId" lay-search lay-verify="required">
				    		...
				    		<c:if test="${type == 'edit'}">
				    		<option value="0" <c:if test="${contentMap.catId==0 }">selected="selected"</c:if>>/</option>
	 						<c:forEach items="${columnList }" var="item">
	 							<c:if test="${item.parentId==0 }">
		 							<option value="${item.catId }" <c:if test="${item.catId==contentMap.catId }">selected="selected"</c:if>>|-${item.catName }</option>
		 							<c:forEach items="${columnList }" var="items">
		 								<c:if test="${item.catId==items.parentId }">
		 									<option value="${items.catId }" <c:if test="${items.parentId!=0 }">style="text-indent:2em;"</c:if> 
		 										<c:if test="${items.catId==contentMap.catId }">selected="selected"</c:if>>|-${items.catName }</option>
		 									<c:forEach items="${columnList }" var="itemss">
			    								<c:if test="${items.catId==itemss.parentId }">
 													<option value="${itemss.catId }" <c:if test="${itemss.parentId!=0 }">style="text-indent:4em;"</c:if> 
 														<c:if test="${itemss.catId==contentMap.catId }">selected="selected"</c:if>>|-${itemss.catName }</option>
			    								</c:if>
						    				</c:forEach>
					    				</c:if>
					    			</c:forEach>
	            	...
	            	<div class="layui-form-item">
						...
						<div class="layui-input-block">
							<input type="text" name="title" id="title" value="${contentMap.title }" lay-verify="required" autocomplete="off" class="layui-input"/>
						</div>
					</div>
					<div class="layui-form-item">
						<label class="layui-form-label">副标题：</label>
						<div class="layui-input-block">
							<input type="text" name="subTitle" id="subTitle" value="${contentMap.subTitle }" autocomplete="off" class="layui-input"/>
						</div>
					</div>
					<div class="layui-form-item">
						<label class="layui-form-label">关键字：</label>
						<div class="layui-input-block" >
							 <input type="text" name="keywords" id="keywords" value="${contentMap.keywords }" autocomplete="off" class="layui-input"/>
						</div>
					</div>
					<div class="layui-form-item">
					...
					<div class="layui-form-item">
						...
						<div class="layui-input-block" >
							 <input type="text" name="textUrl" id="textUrl" value="${contentMap.textUrl }" autocomplete="off" class="layui-input"/>
						</div>
					</div>
					<div class="layui-form-item">
						...
						<div class="layui-input-block" >
							...
							<div style="margin-top: 7px;padding: 0;height: 100px;">
                                <img src="${contentMap.picUrl }" id="imgShow" width="100px" style="margin-left: 40px;"/>
					...
					<div class="layui-form-item">
						...
						<div class="layui-input-block" >
							 <input type="text" name="author" id="author" value="${contentMap.author }" autocomplete="off" class="layui-input">
						</div>
					</div>
					<div class="layui-form-item">
						<label class="layui-form-label">简介：</label>
						<div class="layui-input-block">
							<textarea id="summary" name="summary" class="layui-textarea">${contentMap.summary}</textarea>
						</div>
					</div>
					<div class="layui-form-item">
						<label class="layui-form-label">页面标题：</label>
						<div class="layui-input-block">
							<input type="text" name="webTitle" id="webTitles" value="${contentMap.webTitle}" autocomplete="off" class="layui-input">
						</div>
					</div>
					<div class="layui-form-item">
						<label class="layui-form-label">页面关键字：</label>
						<div class="layui-input-block">
							<input type="text" name="keywords" id="key" value="${contentMap.keywords}"  autocomplete="off" class="layui-input">
						</div>
					</div>
					<div class="layui-form-item">
						<label class="layui-form-label">页面描述：</label>
						<div class="layui-input-block">
							<textarea id="webDescription" name="webDescription" class="layui-textarea">${contentMap.webDescription}</textarea>
						</div>
					</div>
					<div class="layui-form-item">
		...
		<div>
			...
			<c:if test="${type=='edit' }">
			<button class="layui-btn" lay-submit lay-filter="saveBtn">修改内容</button>
			</c:if>
		</div>
	</form>
	</div>
