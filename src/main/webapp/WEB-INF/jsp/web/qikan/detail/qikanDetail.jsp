...
<pxkj:ContentPage materPageId="WebMaster">
	...
	<pxkj:Content contentPlaceHolderId="content">
		<div class="qk-detail">
			...
			<div class="qk-ins">
				...
				<c:if test="${yuanPaperPrice+0 != paperPrice+0 }">
					<span style="color:#888;margin-left: 15px;text-decoration:line-through;font-size: 14px;display:none" id="yuanPPrice">${yuanPaperPrice}</span>
				</c:if>
				<c:if test="${yuanEbookPrice != ebookPrice }">
					<span style="color:#888;margin-left: 15px;text-decoration:line-through;font-size: 14px;" id="yuanEPrice">${yuanEbookPrice}</span>
				</c:if>
				...
				<p class="type1" id="type1">
					...
					<c:choose>
						...
						<c:when test="${sumType eq '4' && isSalePaper>0}">
							<span data-va="4" data-id="${id }" data-name="${name }" data-price="${paperPrice }">双刊</span>
						</c:when>
				...
				<p style="display:none;" class="type2" id="type2">
					...
					<c:choose>
						<c:when test="${sumType eq '1' && isSaleEbook>0 }">
							<span data-val="2" data-id="${id }" data-name="${name }" data-price="${ebookPrice }">上半年</span>
						</c:when>
						<c:when test="${sumType eq '2' && isSaleEbook>0 }">
							<span data-val="3" data-id="${id }" data-name="${name }" data-price="${ebookPrice }">下半年</span>
						</c:when>
						<c:when test="${sumType eq '3' && isSaleEbook>0 }">
							<span data-val="4" data-id="${id }" data-name="${name }" data-price="${ebookPrice }">全年</span>
						</c:when>
						<c:when test="${sumType eq '4'  }">
							<span data-val="4" data-id="${id }" data-name="${name }" data-price="${ebookPrice}">双刊</span>
						</c:when>
						<c:when test="${isSaleEbook > 0 }">
		...
		<c:if test="${isBuySend > 0 }">
			<div class="qk-neiron">
				<span class="songSpan">促  销</span>
				<div class="song">${buySendList[0].name }</div>
	...
	<!-- 期次 -->
	<input type="hidden" value=${desc } id="period">
	<!-- ------------------------------------- -->
	<!-- 商品个数 -->
