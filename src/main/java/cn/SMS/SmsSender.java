...
import cn.SMS.SmsSender.SmsState;
...
public class SmsSender {
	...
	public SmsState send(SmsSetting setting,String phone,String message,int priority){
		...
		try {
			...
			if(setting.getPassword().length()>8||StringHelper.IsNullOrEmpty(setting.getServer())) {
				return sendBySDK(setting, phone, message);
			}
	 ...
	 * @return
	 */
	public SmsState sendBySDK(SmsSetting setting,String phone,String message) {
		if(setting==null){
			return SmsState.NoSetting;
		}
		if(StringHelper.IsNullOrEmpty(phone)){
		...
		try {
			...
			SmsSDKClient client = new SmsSDKClient(setting.getAccount(),setting.getPassword());
			
			SmsSingleRequest request = new SmsSingleRequest();
			request.setMobile(phone);
			request.setContent(message);
			request.setRequestValidPeriod(30);
